...
export default class extends SteveCommand {
	...
	public constructor(store: CommandStore, file: string[], directory: string) {
		super(store, file, directory, {
			aliases: ['ds', 'discordstatus', 'isdiscordbroke', 'isdiscordborked', 'wherediscord'],
			cooldown: 60,
	...
	public async run(msg: KlasaMessage): Promise<Message> {
		...
		request(url, options, (error, res, currentStatus: DiscordStatus) => {
			if (error) {
	...
	const embed = new MessageEmbed()
		...
		.setTimestamp()
		.setFooter(`Last changed ${formatDate(new Date(currentStatus.page.updated_at))}`)
		.setColor('BLURPLE');
