...
export default class extends SteveCommand {
	public constructor(store: CommandStore, file: string[], directory: string) {
		super(store, file, directory, {
			...
			usage: '<faxNumber:fax> <faxContent:string>'
		});

		this.createCustomResolver('fax', (str, possible, msg) => {
			const faxUsers = getFaxUsers(msg.client);
			if (!faxUsers.has(str)) throw 'Lorna doesn\'t have that number in her address book';
			return str;
		});
	}

	public async run(msg: KlasaMessage, [number, message]: string[]): Promise<Message> {
		const faxUsers = getFaxUsers(msg.client);
		const recipient = faxUsers.get(number);
		const faxDest = msg.client.channels.cache.get(recipient.settings.get(UserSettings.Fax.Channel)) as TextChannel;

		message = Util.cleanContent(message, msg).replace(/<a?(:[a-zA-Z0-9_-]{2,}:)\d+>/g, '$1');

		const response = msg.channel.send(`Dialing ${number}`);
